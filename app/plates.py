...
async def get_watchlists_route(request: Request = None, user = None):
    """ดึงรายการทะเบียนที่ต้องการติดตาม"""
    try:
        # ดึง user_id และตรวจสอบว่าเป็น admin หรือไม่
        user_id = None
        is_admin = None == "admin" if hasattr(request.state, "role") else False
...
async def get_alerts_route(request: Request, status: Optional[str] = None, user = None):
    """ดึงรายการแจ้งเตือน"""
    try:
